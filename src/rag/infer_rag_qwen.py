# src/exp2/rag/infer_rag_qwen.py
# -*- coding: utf-8 -*-
"""
实验2：RAG + Qwen 推理脚本（生成最终回答，不做RAGAS评估、不保存中间RAG结果）

目标：
- 输入：experiments/exp2/data/qa_50.json（精选测试集，JSON数组）
- 流程：RAG(retrieval + RRF) -> Prompt组装 -> Qwen 生成回答
- 输出：experiments/exp2/results/rag/
  字段：query / answer / LLM_answer（answer仅用于落盘对照，不喂给LLM）
- 模型：Qwen2.5 3B / 7B 用环境变量控制，输出文件名体现模型与RAG关键参数
...
运行方式（项目根目录）：
  ...
  $env:RRF_TOPK="20"
  $env:RRF_K="60"
  $env:RAG_CTX_TOPK="5"           # 最终喂给LLM的context条数（top5/top10）
  ...
  # LLM（3B/7B 切换）
  $env:QWEN_SIZE="3b"             # 3b / 7b
  # $env:QWEN_SIZE="7b"
  $env:LLM_DEVICE="cuda"          # auto/cpu/cuda/cuda:0
  $env:LLM_MAX_NEW_TOKENS="256"
  $env:LLM_TEMPERATURE="0.2"
  $env:LLM_TOP_P="0.8"

  # Sanity（可选）
  $env:RAG_SANITY_N="30"          # 先跑前30条；跑全量就 Remove-Item Env:RAG_SANITY_N

  python -m src.exp2.rag.infer_rag_qwen
"""
...
# 输入数据配置
# -------------------------
DEFAULT_DATA_FILE = os.getenv("RAG_QA_FILE", "qa_50.json")  # -> experiments/exp2/data/qa_50.json
...
RAG_CTX_TOPK = int(os.getenv("RAG_CTX_TOPK", "5")) # 最终喂给LLM的context条数（top5/top10）
...
# 你可以用环境变量覆盖 MODEL_ID；否则按 size 自动选择
...
# -------------------------
# Prompt（留出位置：后续你把 prompt 文件放到 experiments/exp2/prompts/）
# -------------------------
...
# =========================================================
# 2) 工具函数：设备选择 / 读取prompt / 读取数据 / RAG检索 / LLM生成
# =========================================================
...
def load_prompt_text() -> str:
    """
    优先从 experiments/exp2/prompts/rag_qa.txt 读取；
    不存在则使用 DEFAULT_RAG_PROMPT。
    """
...
def load_testset(qa_file: str) -> List[Dict[str, str]]:
    """
    读取 qa_file（JSON数组）
    - 每条至少包含 query
    - answer 作为参考保留，但【绝不】喂给LLM（防止泄露）
    """
    p = data_path(qa_file)
    if not p.exists():
    ...
    for obj in data:
        ...
        if isinstance(q, str) and q.strip():
            items.append({
                "query": q.strip(),
                "answer": (obj.get("answer", "") or "").strip(),  # 仅落盘对照
            })
...
# -------------------------
# Embedding：BGE-large query embedding
# -------------------------
class STEmbedder:
...
def format_contexts(ctxs: List[str]) -> str:
    """
    将 contexts 格式化为可读文本块（给 LLM 的引用材料）
    """
    lines = []
...
def build_llm_input(tokenizer, prompt_tpl: str, query: str, ctxs: List[str]) -> str:
    """
    构造 LLM 输入（chat template 优先）
    注意：严格不包含参考答案 answer，避免泄露
    """
...
# =========================================================
# 3) 主流程：加载 -> RAG检索 -> Prompt -> LLM生成 -> 保存
# =========================================================
def run_rag_qwen(qa_file: str) -> str:
    # ---------- 读取数据与prompt ----------
    items = load_testset(qa_file)
    ...
    # ---------- 输出路径 ----------
    out_dir = ensure_dir(results_path() / OUT_SUBDIR)  # -> experiments/exp2/results/rag
    ...
    # ---------- 打印关键配置 ----------
    print("=" * 90)
    print("[INFO] 实验：Exp2 RAG + Qwen 推理（不评估，不保存中间RAG）")
    ...
    for idx, obj in enumerate(items, start=1):
        ...
        # 1) RAG 检索（不保存中间结果）
        bm25_ctxs = bm25_search(es, BM25_INDEX, query, BM25_TOPK)
        ...
        # 2) Prompt 组装（严格不包含参考答案）
        input_text = build_llm_input(tokenizer, prompt_tpl, query, ctxs_for_llm)
        ...
        results.append({
            "query": query,
            "answer": ref_answer,      # 仅用于结果对照，绝不喂给LLM
...
# =========================================================
# 4) CLI 入口（✅ 无 global，避免 SyntaxError）
# =========================================================
def main():
    parser = argparse.ArgumentParser(description="Exp2 RAG + Qwen Inference (no RAGAS)")
    parser.add_argument(
        "--qa_file",
        type=str,
        default=DEFAULT_DATA_FILE,
        help="input QA json filename under experiments/<exp>/data/ (e.g., qa_50.json)",
    )
    args = parser.parse_args()

    run_rag_qwen(qa_file=args.qa_file)
